...
class LabelChecker():
    ...
    def get_pattern(self, wraped_img: np.ndarray, threshold: int, shielded_areas: list = None):
        ...
        if (isinstance(shielded_areas, list)):
            for area in shielded_areas:
                # 检查输入维度是否合法
                if (len(area) != 4):
                    logging.error("Shielded'area setting error, requires [(x1, x2, y1, y2), ...], But get [..., " + str(
                        area) + ", ...]")
                    continue
                # 检查坐标点是否合法
                ...
                if (
                    ...
                    continue
                # 执行屏蔽
                target_pattern = cv2.rectangle(target_pattern, (area[0], area[1]), (area[2], area[3]), 0,
                                               thickness=cv2.FILLED)

        return target_pattern

    def linear_trans_to(self, img: np.ndarray, x: int, y: int, angle: float, output_size: list = [-1, -1],
                        border_color=[0, 0, 0]):
        '''
        ...
        test_ext = cv2.copyMakeBorder(img, border_h, border_h, border_w, border_w, cv2.BORDER_CONSTANT,
                                      value=border_color)
    ...
    def try_match(self, img1: np.ndarray, img2: np.ndarray, x: int, y: int, angle, shielded_areas: list = None,
                  show_diff=False) -> int:
        '''
    ...
    def fine_tune(self, test: np.ndarray, std: np.ndarray, max_abs_x: int, max_abs_y: int, max_abs_a: float,
                  max_iterations,
        ...
        std_with_border = cv2.copyMakeBorder(std, border_h, border_h, border_w, border_w, cv2.BORDER_CONSTANT,
                                             value=(0))
    ...
    def match_template_to_target_partitioned(self, template_pattern: np.ndarray, target_pattern: np.ndarray,
        ...
        if (isinstance(shielded_areas, list)):
            for area in shielded_areas:
                # 执行屏蔽
                template_pattern = cv2.rectangle(template_pattern, (area[0], area[1]), (area[2], area[3]), 0,
                                                 thickness=cv2.FILLED)
                target_pattern = cv2.rectangle(target_pattern, (area[0], area[1]), (area[2], area[3]), 0,
                                               thickness=cv2.FILLED)
...
class InkDefectDetector():
    ...
    def detect(self, img: np.ndarray, confidence_thre: float = 0.2, template_defects: list = []) -> list:
        ...
        bordered_img = cv2.copyMakeBorder(scaled_img, 0, border_h, 0, border_w, cv2.BORDER_CONSTANT,
                                          value=(255, 255, 255))
